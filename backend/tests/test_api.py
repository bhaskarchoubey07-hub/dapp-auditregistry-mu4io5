...
def test_full_audit_lifecycle():
    ...
    try:
        ...
        with main.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("DELETE FROM blockchain_records WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM audit_certificates WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM audit_records WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM benford_results WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM compliance_findings WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM anomalies WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM reconciliations WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM transactions WHERE session_id = ?;", (session_id,))
            cur.execute("DELETE FROM audit_sessions WHERE id = ?;", (session_id,))
            conn.commit()
